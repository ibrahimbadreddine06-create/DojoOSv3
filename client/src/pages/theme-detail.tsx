...
function AddChapterDialog({ topicId, parentId = null, onClose }: any) {
  const [title, setTitle] = useState("");
  const [importance, setImportance] = useState(3);
  const createMutation = useMutation({
    mutationFn: async (data: any) => apiRequest("POST", "/api/learn-plan-items", data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/learn-plan-items", topicId] }); onClose(); },
  });
...
export default function ThemeDetail() {
  ...
  const { data: theme, isLoading: themeLoading } = useQuery<KnowledgeTopic>({ queryKey: ["/api/knowledge-topics/detail", topicId] });
  const { data: chapters = [], isLoading: chaptersLoading } = useQuery<LearnPlanItem[]>({ queryKey: ["/api/learn-plan-items", topicId], enabled: !!topicId });
  const { data: flashcards = [] } = useQuery<Flashcard[]>({ queryKey: ["/api/flashcards/theme", topicId], enabled: !!topicId });
  const { data: knowledgeMetrics = [] } = useQuery<KnowledgeMetric[]>({ queryKey: ["/api/knowledge-metrics", topicId], enabled: !!topicId });
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-semibold" data-testid="text-theme-title">{theme.name}</h1>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 p-6 overflow-y-auto">
                <div className="max-w-3xl mx-auto">
                  <ChapterContentArea chapter={selectedChapter} topicId={topicId} childChapterIds={selectedChapterChildIds} />
                </div>
              </div>
            </div>
          </>
