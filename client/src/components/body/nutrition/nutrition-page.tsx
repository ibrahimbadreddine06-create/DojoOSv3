  ...
  return (
    <div className="p-4 sm:p-6 pb-24 max-w-7xl mx-auto animate-in fade-in duration-700">
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        <Button onClick={() => setLogModalOpen(true)} size="sm" className="gap-1.5 shadow-sm rounded-xl bg-orange-500 hover:bg-orange-600 border-none text-white">
          <Plus className="w-4 h-4" /> Log food
        </Button>
      </div>
      <ModuleGrid widgets={[]} storageKey="moduleGrid_nutrition_v100_fresh_start" />
      <LogIntakeModal isOpen={logModalOpen} onClose={() => setLogModalOpen(false)} />
    </div>
  );
}
