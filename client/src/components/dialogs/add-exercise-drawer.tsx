import React, { useState, useMemo } from "react";
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Search, Plus, Dumbbell } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { ExerciseLibraryItem } from "@shared/schema";
import { ScrollArea } from "@/components/ui/scroll-area";
import { cn } from "@/lib/utils";

interface AddExerciseDrawerProps {
    onSelect: (exerciseId: string) => void;
    trigger?: React.ReactNode;
}

const MUSCLE_FILTERS = [
    { label: "All", value: null },
    { label: "Chest", value: "chest" },
    { label: "Back", value: "upper-back" },
    { label: "Shoulders", value: "front-deltoids" },
    { label: "Biceps", value: "biceps" },
    { label: "Triceps", value: "triceps" },
    { label: "Quads", value: "quadriceps" },
    { label: "Hamstrings", value: "hamstring" },
    { label: "Glutes", value: "gluteal" },
    { label: "Calves", value: "calves" },
    { label: "Abs", value: "abs" },
    { label: "Cardio", value: "cardio" },
];

export function AddExerciseDrawer({ onSelect, trigger }: AddExerciseDrawerProps) {
    const [open, setOpen] = useState(false);
    const [search, setSearch] = useState("");
    const [muscleFilter, setMuscleFilter] = useState<string | null>(null);

    const { data: exercises } = useQuery<ExerciseLibraryItem[]>({
        queryKey: ["/api/exercise-library"],
    });

    const filtered = useMemo(() => exercises?.filter(e => {
        const matchesSearch = !search ||
            e.name.toLowerCase().includes(search.toLowerCase()) ||
            e.targetMuscleGroup?.toLowerCase().includes(search.toLowerCase());
        const matchesMuscle = !muscleFilter || e.targetMuscleGroup === muscleFilter;
        return matchesSearch && matchesMuscle;
    }), [exercises, search, muscleFilter]);

    const handleSelect = (id: string) => {
        onSelect(id);
        setOpen(false);
        setSearch("");
        setMuscleFilter(null);
    };

    return (
        <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
                {trigger || (
                    <Button className="w-full h-12 text-lg font-bold uppercase tracking-tight" variant="secondary">
                        <Plus className="w-5 h-5 mr-2" /> Add Exercise
                    </Button>
                )}
            </SheetTrigger>
            <SheetContent
                side="bottom"
                className="rounded-t-3xl flex flex-col"
                style={{ height: "85vh", paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}
            >
                <SheetHeader className="shrink-0">
                    <SheetTitle>Select Exercise</SheetTitle>
                </SheetHeader>

                <div className="mt-3 shrink-0 relative">
                    <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                    <Input
                        placeholder="Search 800+ exercises..."
                        className="pl-9"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        autoComplete="off"
                    />
                </div>

                {/* Muscle group filter chips */}
                <div className="mt-2 shrink-0 -mx-6 px-6 overflow-x-auto scrollbar-none">
                    <div className="flex gap-2 pb-1">
                        {MUSCLE_FILTERS.map(f => (
                            <button
                                key={f.label}
                                onClick={() => setMuscleFilter(f.value)}
                                className={cn(
                                    "shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition-all border",
                                    muscleFilter === f.value
                                        ? "bg-primary text-primary-foreground border-primary"
                                        : "bg-muted/50 text-muted-foreground border-border/50 hover:bg-muted"
                                )}
                            >
                                {f.label}
                                {f.value && exercises && (
                                    <span className="ml-1 opacity-50">
                                        {exercises.filter(e => e.targetMuscleGroup === f.value).length}
                                    </span>
                                )}
                            </button>
                        ))}
                    </div>
                </div>

                <ScrollArea className="flex-1 -mx-6 px-6 mt-2">
                    <div className="space-y-1.5 pb-4">
                        {filtered?.length === 0 && (
                            <div className="py-12 text-center text-muted-foreground">
                                <Dumbbell className="w-8 h-8 mx-auto mb-2 opacity-20" />
                                <p className="text-sm">No exercises found</p>
                            </div>
                        )}
                        {filtered?.map((exercise) => (
                            <div
                                key={exercise.id}
                                className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/50 cursor-pointer border border-transparent hover:border-primary/10 transition-all"
                                onClick={() => handleSelect(exercise.id)}
                            >
                                <div className="flex items-center gap-3">
                                    <div className="h-12 w-12 rounded-xl overflow-hidden bg-muted flex-shrink-0 flex items-center justify-center relative">
                                        <Dumbbell className="w-5 h-5 text-muted-foreground/40" />
                                        {exercise.imageUrl && (
                                            <img
                                                src={exercise.imageUrl}
                                                alt={exercise.name}
                                                className="absolute inset-0 h-full w-full object-cover"
                                                onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }}
                                            />
                                        )}
                                    </div>
                                    <div>
                                        <h4 className="font-semibold text-sm">{exercise.name}</h4>
                                        <p className="text-xs text-muted-foreground capitalize">{exercise.targetMuscleGroup?.replace(/-/g, " ")}</p>
                                    </div>
                                </div>
                                <Button size="icon" variant="ghost" className="h-8 w-8 flex-shrink-0">
                                    <Plus className="w-4 h-4" />
                                </Button>
                            </div>
                        ))}
                    </div>
                </ScrollArea>
            </SheetContent>
        </Sheet>
    );
}
